  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setCurrentUser({
          id: sessionUser.id,
          full_name: profile?.full_name ?? (sessionUser.user_metadata as any)?.full_name ?? undefined,
            ...
            {messages.map((message, index) => {
              ...
              return (
                <div
                  ...
                  className={`flex ${isUser ? "justify-end items-center" : "justify-start items-center"}`}
                >
                  {!isUser && (
                  ...
                  <div
                    className={`max-w-[85%] whitespace-pre-wrap wrap-break-word rounded-2xl px-4 py-3 text-sm leading-relaxed ${
                      isUser ? "bg-cyan-600 text-white" : "border border-black/10 bg-white text-foreground dark:border-white/10 dark:bg-white/10"
                    }`}
                  >
